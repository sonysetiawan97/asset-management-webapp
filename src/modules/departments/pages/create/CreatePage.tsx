import { moduleName, type CreateModel, type Model } from "@modules/departments/types/Model";
...
const CreatePage = () => {
  ...
  const { data: departmentsData, isLoading: loadingDepartments } = useFindAll<Model>("departments", "departments");

  if (loadingDepartments) return <ContentLoader />;
  ...
  const onSubmit = async (data: CreateModel) => {
    try {
      ...
      enqueueSnackbar(t("modules.departments.create.notification.success"), { variant: "success" });
      navigate(`/${moduleName}`);
    } catch (error: unknown) {
